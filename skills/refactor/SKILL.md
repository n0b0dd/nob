...
- `docs/specs/`             — PRDs (product requirements; PM writes here).
- `docs/design/`            — technical design docs (Tech Lead writes here).
...
Write `{WORKING_DIR}/.nob.yml` (overwrite if exists) using the Write tool. Write a **minimal** config — only `units` is required; everything else (agents, models, parallelism, checkpoint, docs paths) has sensible defaults. Do not emit an `agents` block (it drifts from the defaults and the full reference):

```yaml
# nob configuration (generated by /nob refactor).
