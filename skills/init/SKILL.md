...
Write a **minimal** `.nob.yml` — only `units` is required; everything else (agents, models, parallelism, checkpoint, docs paths) has sensible defaults. Do not emit an `agents` block (it drifts from the defaults and the full reference). Write:

```yaml
# nob configuration (generated by /nob init).
