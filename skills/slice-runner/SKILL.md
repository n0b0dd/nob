...
Follow PM_SKILL instructions in this session. Emit `[PM OUTPUT]` before continuing. Store the extracted block as PM_OUTPUT.
...
Requirements from PM Agent:
{PM_OUTPUT}
Backend Agent is running in parallel — use API contracts from PM Agent output above. No [BACKEND OUTPUT] will be provided.
{if Clarifications: Clarifications from user: {Clarifications from your [INPUTS]}}
...
- If `stack.backend.enabled: false` or workflow is `API→Sync`: skip Backend; emit `[BACKEND OUTPUT]Backend agent was skipped (disabled in config)[/BACKEND OUTPUT]`.
- If `stack.frontend.enabled: false`: skip Frontend; emit `[FRONTEND OUTPUT]Frontend agent was skipped (disabled in config)[/FRONTEND OUTPUT]`.
