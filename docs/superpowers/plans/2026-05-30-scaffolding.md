# Nob Scaffolding Support Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add `/nob init` command that scaffolds a complete runnable fullstack project from scratch — asking the user to describe what they're building, recommending a tech stack, generating working boilerplate, running dependency installation, and writing `CLAUDE.md` + `.nob.yml`.

**Architecture:** A new `Init` workflow type is added to the hub's Step 2 dispatch table. When detected, the hub skips all phases and dispatches a single `init-agent` sub-skill. The init-agent handles the full lifecycle: empty-check → project description → stack recommendation → file generation → install → CLAUDE.md + .nob.yml → output.

**Tech Stack:** Markdown skill files only — no build system or runtime. The init-agent generates Next.js / React+Vite / Vue / Flutter frontends and Express / FastAPI / Go backends.
...
**Modified:**
- `skills/nob/SKILL.md` — add Init to workflow table (line 113), add Init early-exit block (after line 118), add init-agent model default (line 89), add Init terminal summary (Step 4), add `nob/init` fallback branch name to Step 0

**Created:**
- `skills/nob/init-agent/SKILL.md` — complete init agent skill

---

### Task 1: Update hub — Init workflow detection, dispatch, and terminal summary

**Files:**
- Modify: `skills/nob/SKILL.md`

- [ ] **Step 1: Read the hub file to confirm line positions**

Run:
```bash
grep -n "API → Sync\|Do NOT guess\|reviewer: haiku\|Step 4: Print" skills/nob/SKILL.md
```
Expected output includes lines near 113, 118, 89, and 548. Confirm before editing.

- [ ] **Step 2: Add `nob/init` fallback branch name to Step 0**

The hub's Step 0 derives a branch name from `nob/<source-filename>`, but `/nob init` has no source file, so derivation fails. Fix with a fallback.
...
- [ ] **Step 4: Add Init row to workflow table**
...
- [ ] **Step 5: Add Init early-exit block after "Do NOT guess" line**
...
- [ ] **Step 4: Add init-agent model default**
...
- [ ] **Step 7: Add Init terminal summary to Step 4**
...
- [ ] **Step 8: Verify edits are correct**

Run:
```bash
grep -n "Init\|init-agent" skills/nob/SKILL.md
```
Expected: Init appears in the workflow table, early-exit block header, model defaults, and Step 4 summary. init-agent appears in the model defaults and early-exit dispatch.

- [ ] **Step 9: Commit**

```bash
git add skills/nob/SKILL.md
git commit -m "feat: add Init workflow type to nob hub"
```

---

### Task 2: Create skills/nob/init-agent/SKILL.md

**Files:**
- Create: `skills/nob/init-agent/SKILL.md`

- [ ] **Step 1: Create directory**

```bash
mkdir -p skills/nob/init-agent
```

- [ ] **Step 2: Write the complete file**

Write `skills/nob/init-agent/SKILL.md` with this exact content:
...
```bash
wc -l skills/nob/init-agent/SKILL.md
```
Expected: 300+ lines
...
```bash
grep -n "Step [1-8]\|INIT-AGENT OUTPUT\|FRONTEND_TYPE\|BACKEND_TYPE" skills/nob/init-agent/SKILL.md
```
Expected: all 8 steps present, INIT-AGENT OUTPUT appears in Steps 1 and 8, FRONTEND_TYPE and BACKEND_TYPE appear throughout Step 4.

- [ ] **Step 5: Commit**

```bash
git add skills/nob/init-agent/SKILL.md
git commit -m "feat: add init-agent skill for project scaffolding"
```
...
- [ ] **Step 1: Verify hub correctly references init-agent**

```bash
grep -n "init-agent\|Init" skills/nob/SKILL.md
```
Expected output (6 matches minimum):
- workflow table row with `Init`
- early-exit block header `## Init workflow early exit`
- `init-agent/SKILL.md` path reference
- `init-agent` in model defaults
- `Init` in Step 4 terminal summary

- [ ] **Step 2: Verify init-agent path matches hub reference**

The hub references `{SKILL_BASE_DIR}/init-agent/SKILL.md`. Confirm the file exists at:
```bash
ls skills/nob/init-agent/SKILL.md
```
...
```bash
grep "SKILL_BASE_DIR" skills/nob/SKILL.md | grep -v "init-agent\|planner\|pm-agent\|backend-agent\|frontend-agent\|qa-agent\|reviewer"
```
Expected: no output (all SKILL_BASE_DIR references point to known sub-skill paths).

- [ ] **Step 4: Commit verification confirmation**

```bash
git add -p
git commit -m "chore: verify scaffolding implementation complete"
```
If no changes to stage, skip this step.
